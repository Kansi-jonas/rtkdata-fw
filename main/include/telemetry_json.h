...
/* Worst case, computed not guessed: 17 members, ~215 bytes of key text,
 * 4 bytes of punctuation per member, 10 uint64 values at <= 20 digits,
 * 6 uint32 values at <= 10 digits, one boolean (<= 5), "tx":{ prefix and }
 * suffix (7). 215 + 68 + 200 + 60 + 5 + 7 = 555, rounded up to 640. The test
 * suite proves the bound with every counter at its maximum. */
#define TELEMETRY_TX_JSON_MAX 640
